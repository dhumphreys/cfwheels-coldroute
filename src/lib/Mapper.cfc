<cfcomponent output="false">
	<cfinclude template="#$wheelsStruct().webPath#/wheels/global/functions.cfm" />
	...
	<cffunction name="$wheelsStruct" returntype="struct" output="false" access="private" hint="set wheels scope to allow $wheels application variable (1.3 compatility)">
		<cfscript>
			var loc = {};
	...
	<cffunction name="resource" returntype="struct" access="public" hint="Set up singular REST resource">
		...
		<cfscript>
			...
			if (StructKeyExists(arguments, "controller")) {
				...
				switch ($wheelsStruct().resourceControllerNaming) {
	...
	<cffunction name="$addRoute" returntype="void" access="private" hint="Add route to cfwheels, removing useless params">
		<cfscript>
			...
			ArrayAppend($wheelsStruct().routes, arguments);
		</cfscript>
	</cffunction>
