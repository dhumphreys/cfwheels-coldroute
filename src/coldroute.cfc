<cfcomponent output="false">
	<cffunction name="init" returntype="struct" access="public">
		<cfscript>
			...
			this.version = "1.1,1.1.1,1.1.2,1.1.3,1.1.4,1.1.5,1.1.6,1.1.7,1.1.8,1.3,1.3.1";

			// get cfwheels plugin prefix
			loc.prefix = ListChangeDelims(application[$appKey()].webPath & application[$appKey()].pluginPath, ".", "/");

			// initialize coldroute mapper
			application[$appKey()].coldroute = CreateObject("component", "#loc.prefix#.coldroute.lib.Mapper").init();

			// set wheels setting for resource controller naming
			// NOTE: options are singular, plural, or name
			if (NOT StructKeyExists(application[$appKey()], "resourceControllerNaming"))
				application[$appKey()].resourceControllerNaming = "plural";
	...
	<cffunction name="drawRoutes" mixin="application" returntype="struct" output="false" access="public" hint="Start drawing routes">
		...
		<cfreturn application[$appKey()].coldroute.draw(argumentCollection=arguments) />
	</cffunction>
    ...
    <cffunction name="startFormTag" mixin="controller" returntype="string" access="public" output="false" hint="Make it easy for the developer to create forms with the proper http method specified">
    	<cfscript>
			...
			if (application[$appKey()].showErrorInformation && !StructKeyExists(application[$appKey()].namedRoutePositions, arguments.route))
			...
			for (loc.position in ListToArray(application[$appKey()].namedRoutePositions[arguments.route]))
				if (StructKeyExists(application[$appKey()].routes[loc.position], "methods") && ListFindNoCase(application[$appKey()].routes[loc.position].methods, arguments.method))
	...
	<cffunction name="URLFor" mixin="controller" returntype="string" access="public" output="false">
		...
		<cfargument name="$URLRewriting" type="string" required="false" default="#application[$appKey()].URLRewriting#">
		<cfscript>
			var loc = {};
			...
			if (application[$appKey()].showErrorInformation AND arguments.onlyPath AND (Len(arguments.host) OR Len(arguments.protocol)))
			...
			if (arguments.route EQ "" AND arguments.action NEQ "") {
				...
				if (StructKeyExists(loc.cache, loc.key)) {
					...
					loc.iEnd = ArrayLen(application[$appKey()].routes);
					for (loc.i = 1; loc.i LTE loc.iEnd; loc.i++) {
						loc.curr = application[$appKey()].routes[loc.i];
						...
						if (StructKeyExists(loc.curr, "controller") AND loc.curr.controller EQ arguments.controller AND StructKeyExists(loc.curr, "action") AND loc.curr.action EQ arguments.action) {
							arguments.route = application[$appKey()].routes[loc.i].name;
			...
			for (loc.i = 1; loc.i LTE loc.iEnd; loc.i++) {
				...
				else if (application[$appKey()].obfuscateUrls)
			...
			if (arguments.$URLRewriting EQ "Partial")
				loc.returnValue = application[$appKey()].rewriteFile & loc.returnValue;
			else if (arguments.$URLRewriting EQ "Off")
				loc.returnValue = "index.cfm" & loc.returnValue;
			loc.returnValue = application[$appKey()].webPath & loc.returnValue;
	...
	<cffunction name="$findMatchingRoute" mixin="dispatch" returntype="struct" access="public" hint="Help Wheels match routes using path and HTTP method">
		...
		<cfscript>
			...
			loc.iEnd = ArrayLen(application[$appKey()].routes);
			for (loc.i = 1; loc.i LTE loc.iEnd; loc.i++) {
				loc.route = application[$appKey()].routes[loc.i];
				...
				if (NOT StructKeyExists(loc.route, "regex"))
					loc.route.regex = application[$appKey()].coldroute.patternToRegex(loc.route.pattern);
				...
				if (REFindNoCase(loc.route.regex, arguments.path) OR (arguments.path EQ "" AND loc.route.pattern EQ "/")) {
					loc.returnValue = Duplicate(application[$appKey()].routes[loc.i]);
					break;
				}
			}
	...
	<cffunction name="$registerNamedRouteMethods" mixin="controller" returntype="void" access="public" hint="Filter that sets up named route helper methods">
		<cfscript>
			var loc = {};
			for (loc.key in application[$appKey()].namedRoutePositions) {
	...
	<cffunction name="$namedRouteMethod" mixin="controller" returntype="string" access="public" output="false" hint="Body of all named route helper methods">
		<cfscript>
			...
			if (StructKeyExists(application[$appKey()].namedRoutePositions, arguments.route)) {
				loc.routePos = application[$appKey()].namedRoutePositions[arguments.route];
				...
				loc.route = application[$appKey()].routes[loc.pos];
				...
				for (loc.i = 1; loc.i LTE loc.iEnd; loc.i++) {
					...
					if (StructKeyExists(loc, "value")) {
						...
						// if object, do special processing
							...
							if (loc.value.isNew()) {
								if (StructKeyExists(application[$appKey()].namedRoutePositions, pluralize(arguments.route))) {
    ...
    <cffunction name="$initControllerObject" mixin="controller" returntype="any" access="public" output="false">
        ...
        <cfscript>
            ...
            if (!ListFindNoCase(application[$appKey()].existingHelperFiles, arguments.name) && !ListFindNoCase(application[$appKey()].nonExistingHelperFiles, arguments.name))
            {
                if (FileExists(ExpandPath("#application[$appKey()].viewPath#/#LCase(ListChangeDelims(arguments.name, '/', '.'))#/helpers.cfm")))
                    loc.helperFileExists = true;
                if (application[$appKey()].cacheFileChecking)
                {
                    if (loc.helperFileExists)
                        application[$appKey()].existingHelperFiles = ListAppend(application[$appKey()].existingHelperFiles, arguments.name);
                    else
                        application[$appKey()].nonExistingHelperFiles = ListAppend(application[$appKey()].nonExistingHelperFiles, arguments.name);
                }
            }
            if (ListFindNoCase(application[$appKey()].existingHelperFiles, arguments.name) || loc.helperFileExists)
                $include(template="#application[$appKey()].viewPath#/#ListChangeDelims(arguments.name, '/', '.')#/helpers.cfm");
    ...
    <cffunction name="$callAction" mixin="controller" returntype="void" access="public" output="false">
        ...
        <cfscript>
            ...
            if (Left(arguments.action, 1) == "$" || ListFindNoCase(application[$appKey()].protectedControllerMethods, arguments.action))
            ...
            if (!$performedRenderOrRedirect())
            {
                ...
                {
                    if (FileExists(ExpandPath("#application[$appKey()].viewPath#/#LCase(ListChangeDelims(variables.$class.name, '/', '.'))#/#LCase(arguments.action)#.cfm")))
                    {
                        $throw(object=e);
                    }
                    else
                    {
                        if (application[$appKey()].showErrorInformation)
                        {
                        ...
                        else
                        {
                            ...
                            $includeAndOutput(template="#application[$appKey()].eventPath#/onmissingtemplate.cfm");
                            $abort();
                        }
                    }
                }
    ...
    <cffunction name="$generateIncludeTemplatePath" mixin="controller" returntype="string" access="public" output="false">
        ...
        <cfargument name="$baseTemplatePath" type="string" required="false" default="#application[$appKey()].viewPath#" />
    ...
    <cffunction name="$objectFileName" mixin="global" returntype="string" access="public" output="false">
        ...
        <cfscript>
            ...
            if (!ListFindNoCase(application[$appKey()].existingObjectFiles, loc.fullObjectPath) && !ListFindNoCase(application[$appKey()].nonExistingObjectFiles, loc.fullObjectPath))
            {
                ...
                if (application[$appKey()].cacheFileChecking)
                {
                    if (loc.objectFileExists)
                        application[$appKey()].existingObjectFiles = ListAppend(application[$appKey()].existingObjectFiles, loc.fullObjectPath);
                    else
                        application[$appKey()].nonExistingObjectFiles = ListAppend(application[$appKey()].nonExistingObjectFiles, loc.fullObjectPath);
                }
            }
            if (ListFindNoCase(application[$appKey()].existingObjectFiles, loc.fullObjectPath) || loc.objectFileExists)
	...
	<cffunction name="$appKey" returntype="string" access="private" output="false">
		<cfscript>
		var loc = {};
